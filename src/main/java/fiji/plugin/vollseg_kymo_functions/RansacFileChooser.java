...
public class RansacFileChooser extends JPanel {
	...
	protected class MeasureserialListener implements ActionListener {
		...
		public void actionPerformed(final ActionEvent arg0) {
			...
			new Load_ransac_fits(AllMovies).run(null);
		parent.dispose();
	...
	protected void Done(final Frame parent) {
		...
		if (!Batchmoderun)
			new Load_ransac_fits(Tracking.loadMT(new File(chooserA.getSelectedFile().getPath())),
					chooserA.getSelectedFile()).run(null);
		
		if (Simplefile)
			new Load_ransac_fits(Tracking.loadsimple(new File(chooserA.getSelectedFile().getPath())),
					chooserA.getSelectedFile()).run(null);
